...
select
    ...
    round(cast(count(lead_id)::float / nullif(count(distinct visitor_id)::float, 0) * 100 as numeric), 2) as lcr,
	round(cast(count(amount)::float / nullif(count(lead_id)::float, 0) * 100 as numeric), 2) as lc
    from advert
group by 1, 2, 3, 4, 5
order by 1;
...
select 
	...
	round(cast(count(amount)::float / count(lead_id)::float * 100 as numeric), 2) as lc
from conv_rate
;
...
with main1 as (
	select
	 ...
	 from sessions s
	 left join vk_ads vk
	 on vk.utm_source = s.source
	 and vk.utm_medium = s.medium
	 and vk.utm_campaign = s.campaign
	 left join ya_ads ya
	 on ya.utm_source = s.source
	 and ya.utm_medium = s.medium
	 and ya.utm_campaign = s.campaign
	),
...
/* Extra вычисления по метрикам: cpu, cpl. cppu, roi */

/* Переиспользуем запросы для поиска топ-15 платных кликов по выручке */
with vk_and_yandex as (
	select 
		to_char(campaign_date, 'YYYY-MM-DD') as campaign_date,
		utm_source,
